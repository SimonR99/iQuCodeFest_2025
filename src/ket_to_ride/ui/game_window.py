...
class GameWindow:
    ...
    def draw_sidebar(self) -> None:
        ...
        self.draw_sidebar_section(y_offset, "CURRENT PLAYER", [
        ...
        if current_player.missions:
            mission_section_height = self.draw_missions_section(y_offset)
            y_offset += mission_section_height + 20
        else:
            no_mission_height = self.draw_no_missions_section(y_offset)
        ...
        turn_actions = [
            ...
            "",
            "Available Actions:",
            "• Draw gate cards (2 max)" if self.cards_drawn_this_turn < self.max_cards_per_turn else "• Cards limit reached",
            "• Draw mission cards (M)" if self.cards_drawn_this_turn == 0 else "• Can't draw missions",
            "• Claim selected route" if self.selected_route_idx is not None else "• Select a route first",
        ]
        
        turn_section_height = self.draw_sidebar_section(y_offset, "TURN ACTIONS", turn_actions)
        ...
        if self.selected_route_idx is not None:
            ...
            route_section_height = self.draw_sidebar_section(y_offset, "SELECTED ROUTE", route_info)
            y_offset += route_section_height + 20
        
        # Game Status Section (if space allows)
        remaining_space = self.sidebar_rect.bottom - (self.sidebar_rect.y + y_offset)
        if remaining_space > 150:
            game_status = [
                f"Turn: {self.game_state.turn_number}",
                f"Gate deck: {len(self.game_state.deck)} cards",
                f"Mission deck: {len(self.game_state.mission_deck)} cards",
            ]
            ...
            game_section_height = self.draw_sidebar_section(y_offset, "GAME STATUS", game_status)
        ...
        if remaining_space > 100:
            ...
            self.draw_sidebar_section(y_offset, "CONTROLS", controls, (80, 80, 120))
    
    def draw_sidebar_section(self, y_start: int, title: str, content: List[str], title_color=None) -> int:
        """Draw a sidebar section with title and content. Returns the height used."""
        if title_color is None:
            title_color = self.ACCENT_COLOR
            
        section_x = self.sidebar_rect.x + 10
        section_width = self.sidebar_rect.width - 20
        
        # Draw section title background
        title_height = 25
        title_rect = pygame.Rect(section_x, self.sidebar_rect.y + y_start, section_width, title_height)
        pygame.draw.rect(self.screen, title_color, title_rect)
        pygame.draw.rect(self.screen, self.BORDER_COLOR, title_rect, 1)
        ...
        for i, line in enumerate(content):
            if line:  # Skip empty lines for spacing
                ...
                if line.startswith("•"):
                    ...
                    # Make labels slightly bolder by using regular font
                    font = self.font if len(line) < 25 else self.small_font
                
                text = font.render(line, True, text_color)
                self.screen.blit(text, (section_x + 5, self.sidebar_rect.y + content_y))
    ...
    def draw_missions_section(self, y_start: int) -> int:
        """Draw the missions section with visual mission cards. Returns height used."""
        current_player = self.game_state.get_current_player()
        if not current_player or not current_player.missions:
            return 0
        ...
        for i, mission in enumerate(current_player.missions):
            card_rect = pygame.Rect(section_x + 5, self.sidebar_rect.y + card_y, section_width - 10, card_height)
            ...
            # Draw status symbol
            status_text = self.font.render(status_symbol, True, status_color)
            ...
            start_text = self.small_font.render(start_cities, True, text_color)
            self.screen.blit(start_text, (card_rect.x + 30, card_rect.y + 8))
            ...
            transform_surface = self.small_font.render(transform_text, True, text_color)
            self.screen.blit(transform_surface, (card_rect.x + 30, card_rect.y + 25))
            
            # Line 3: Points
            points_text = f"Points: {mission.points}"
            points_surface = self.small_font.render(points_text, True, text_color)
            self.screen.blit(points_surface, (card_rect.x + 30, card_rect.y + 42))
            
            # Draw points value prominently on the right
            points_value = str(mission.points)
            points_value_surface = self.font.render(points_value, True, status_color)
            points_value_rect = points_value_surface.get_rect(right=card_rect.right - 10, centery=card_rect.centery)
            self.screen.blit(points_value_surface, points_value_rect)
    ...
    def draw_no_missions_section(self, y_start: int) -> int:
        ...
        title_height = 25
        title_rect = pygame.Rect(section_x, self.sidebar_rect.y + y_start, section_width, title_height)
        pygame.draw.rect(self.screen, (120, 120, 120), title_rect)  # Gray for no missions
        pygame.draw.rect(self.screen, self.BORDER_COLOR, title_rect, 1)
        ...
        card_height = 80
        card_rect = pygame.Rect(section_x + 5, self.sidebar_rect.y + y_start + title_height + 10, section_width - 10, card_height)
        ...
        for x in range(card_rect.left, card_rect.right, total_length):
            if x + dash_length <= card_rect.right:
                pygame.draw.line(self.screen, self.BORDER_COLOR, (x, card_rect.top), (x + dash_length, card_rect.top), 2)
                pygame.draw.line(self.screen, self.BORDER_COLOR, (x, card_rect.bottom), (x + dash_length, card_rect.bottom), 2)
        
        # Left and right borders
        for y in range(card_rect.top, card_rect.bottom, total_length):
            if y + dash_length <= card_rect.bottom:
                pygame.draw.line(self.screen, self.BORDER_COLOR, (card_rect.left, y), (card_rect.left, y + dash_length), 2)
                pygame.draw.line(self.screen, self.BORDER_COLOR, (card_rect.right, y), (card_rect.right, y + dash_length), 2)
        ...
        for line in instruction_lines:
            if line:
                ...
                font = self.small_font if line != "No missions yet!" else self.font
